Shader "Custom/WaterMobile"
{
	Properties
	{
		...
		_ColSurface("Water Surface Color", Color) = (0.46, 0.80, 0.83)
		_ColShore("Shore Color Tint", Color) = (0.15, 0.76, 0.60)
		_ThresDeepest("Deepest If Depth Exceeds This", Range(0.01, 0.99)) = 0.6
		_Smoothness("Color Transition Smoothness", Range(0,10))= 0.76
		_Extinction("Color Extinction Rate Under Water", Range(0.0, 1.0)) = 0.67

		[Header(Wind And Waves)]
		_VecWind("Velocity of Wind (XY)", Vector) = (1, 1, 0, 0)
		_WaveIntensity("Wave Intensity", Range(0.1, 5)) = 2
		_TexNormal("Bump Normal Texture", 2D) = ""{}


		[Header(Directional Light)]
		_DirDirectionalLight("Light Direction", Vector) = (2, -1, 2)
		_ColDirectionalLight("Light Color", Color) = (1, 1, 1)
		_Shininess("Shininess", Range(0, 3)) = 0.26
		...
		_FoamDepthMax("Max Depth Foam Appears", Range(0, 1)) = 0.3
	...
	SubShader
	{
		...
		Pass
		{
			...
			// For Unity 4.X, uncomment these codes.
			/*
			#define unity_ObjectToWorld _Object2World
			
			float4 UnityObjectToClipPos(float3 posObj){
				return UnityObjectToClipPos(float4(posObj, 1.0));
			}
			*/
			...
			*/
			...
			/* Discard GrabPass and enable blend mode, these calculations now need to be done in pixel shader directly.
...
/////////////////////////////////////////////////////////////
				...
				// Refraction.
				float cosTheta = dot(float3(0,1,0), dirToCameraWorld);
				half factorRefr = saturate(depth * _Extinction / (cosTheta * _Smoothness + 0.2));
				half3 colRefrNoFloorColor = factorRefr * colLit * normalize(colSurf); 
				...
				half4 foamsIntensity = Foam(depth, wavesIntensity);
           		...
           		half3 col = (1-fresnelR)*colRefrNoFloorColor + fresnelR*colRefl + colSpec;
				...
				return float4(col, (1-fresnelR)*(1-factorRefr));
			}
			ENDCG
		}
	}
